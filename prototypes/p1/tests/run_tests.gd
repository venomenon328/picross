...
func test_gesture() -> void:
	...
	p.undo()
	check(p.cells.slice(0, 6) == [0, 1, 2, 1, 1, 0], "eraser undo restores mixed exact values")
...
func test_history() -> void:
	...
	var before: Array[int] = p.cells.duplicate()
	check(not p.commit([{"index": 0, "before": -1, "after": 1}, {"index": 42, "before": -1, "after": 1}]) and p.cells == before, "stale action cannot partly commit")
